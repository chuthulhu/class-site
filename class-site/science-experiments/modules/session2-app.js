// session2-app.js - 2차시 페이지 모듈 (테스트용) - 공유 hub-* 모듈 재사용
...
function toast(msg,bg='bg-gray-800'){ const t=qs('toast-notification'); if(!t) return; clearTimeout(t.__timer); t.textContent=msg; t.className=`fixed bottom-5 right-5 text-white py-3 px-5 rounded-lg shadow-lg transition-opacity duration-300 no-print ${bg}`; t.style.opacity=1; t.__timer=setTimeout(()=>{ t.style.opacity=0; },1800); }
...
// 저장/로드 (세션2 특화)
function loadAll(){ const raw=localStorage.getItem(STORAGE_KEY); if(!raw) return; try{ const data=JSON.parse(raw); Object.entries(data).forEach(([k,v])=>{ if(k==='sources' && Array.isArray(v)){ const c=qs('sources_container'); if(c) v.forEach(s=>addSourceElement(c,s)); } else { const el=qs(k); if(el) el.value=v; } }); }catch{} }
function saveAll(){ const obj={}; document.querySelectorAll('#tab4 input,#tab4 textarea,input#student_grade,input#student_class,input#student_number,input#student_name').forEach(el=>obj[el.id]=el.value); const c=qs('sources_container'); if(c) obj.sources=serializeSources(c); try{ localStorage.setItem(STORAGE_KEY, JSON.stringify(obj)); }catch{} }
...
// 제출
function validateStudent(){ const grade=parseInt((qs('student_grade')?.value||'').trim(),10); const klass=parseInt((qs('student_class')?.value||'').trim(),10); const number=parseInt((qs('student_number')?.value||'').trim(),10); const name=(qs('student_name')?.value||'').trim(); if(Number.isNaN(grade)||grade<1||grade>3){ toast('학년 1~3'); qs('student_grade')?.focus(); return null; } if(Number.isNaN(klass)||klass<1||klass>20){ toast('반 1~20'); qs('student_class')?.focus(); return null; } if(Number.isNaN(number)||number<1||number>40){ toast('번호 1~40'); qs('student_number')?.focus(); return null; } if(!name){ toast('이름 입력'); qs('student_name')?.focus(); return null; } return { grade, klass, number, name }; }
...
function bindSubmit(){ const downloadBtn=qs('download_zip_btn'); const retryBtn=qs('retry_upload_btn'); const printBtn=qs('print_btn'); const previewBtn=qs('preview_newtab_btn'); const statusEl=qs('submit_status'); const progressC=qs('progress_container'); const bar=qs('progress_bar'); const txt=qs('progress_text'); let lastZipBlob=null; let lastMeta=null; let lastPayloadBuilder=null; let lastZipFilename=null;
  ...
  function setProgress(p,l=''){ if(bar) bar.style.width=`${Math.min(100,Math.max(0,p))}%`; if(txt) txt.textContent=l; }
  ...
  function getReportFilename(){ const g=(qs('student_grade')?.value||'1').trim(); const c=(qs('student_class')?.value||'0').trim().padStart(2,'0'); const n=(qs('student_number')?.value||'0').trim().padStart(2,'0'); const id=`${g}${c}${n}`; const name=(qs('student_name')?.value||'이름없음').trim(); return `${id}_${name}_스마트모빌리티_제안서_2차시`; }
  function printReport(){ const orig=document.title; document.title=getReportFilename(); window.print(); setTimeout(()=>{ document.title=orig; },400); }
  function openPreview(){ try{ const inner=qs('report-content')?.innerHTML||''; const now=new Date(); const ts=`${now.getFullYear()}${String(now.getMonth()+1).padStart(2,'0')}${String(now.getDate()).padStart(2,'0')}-${String(now.getHours()).padStart(2,'0')}${String(now.getMinutes()).padStart(2,'0')}${String(now.getSeconds()).padStart(2,'0')}`; const title=`미리보기-2차시-${ts}`; const html=generateDocHtml(title, inner); const blob=new Blob([html],{type:'text/html'}); const url=URL.createObjectURL(blob); window.open(url,'_blank'); setTimeout(()=>URL.revokeObjectURL(url),10000);}catch{}
  }
  async function attempt(){ const form=validateStudent(); if(!form) return; retryBtn?.classList.add('hidden'); showProgress(true); setProgress(8,'문서 준비'); setStatus('문서 생성 중'); const section=calcSectionCode(form.grade, form.klass); const studentId=calcStudentId(form.grade, form.klass, form.number); const nowParts=new Intl.DateTimeFormat('ko-KR',{timeZone:'Asia/Seoul',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false}).formatToParts(new Date()); const parts=Object.fromEntries(nowParts.map(p=>[p.type,p.value])); const ts=`${parts.year}${parts.month}${parts.day}_${parts.hour}${parts.minute}${parts.second}`; const baseName=`${studentId}_${form.name}_${qs('preview_main_title')?.textContent?.trim()||'보고서'}_${ts}`;
    const payloadBuilder=(b64, zipFilename)=>({ studentId, subject: document.querySelector('meta[name="submission-subject"]')?.content||'과학탐구실험2', activity:(document.querySelector('meta[name="submission-activity"]')?.content||'수행3_2차시'), section, files:[{ filename: zipFilename, contentBase64:b64, mime:'application/zip' }] });
    try { const { zipBlob, zipFilename } = await buildZip({ buildHtml:(t,i)=>generateDocHtml(t,i), filenameBase: baseName, reportSelector:'#report-content', onProgress:(p,l)=>setProgress(p,l), onStatus:setStatus }); lastZipBlob=zipBlob; lastZipFilename=zipFilename; lastPayloadBuilder=payloadBuilder; lastMeta={ zipFilename }; const { data } = await submitBuiltZip({ zipBlob, zipFilename, payloadBuilder, onProgress:(p,l)=>setProgress(p,l), onStatus:setStatus }); setProgress(88,'다운로드'); setStatus('다운로드 중'); await downloadWithFallback({ blob:zipBlob, filename:zipFilename, onStatus:setStatus, onToast:(m)=>toast(m) }); setProgress(100,'완료'); setTimeout(()=>showProgress(false),1200); toast('제출 & 다운로드 완료','bg-green-600'); }catch(e){ setStatus('실패: '+e.message); toast('실패: '+e.message,'bg-red-600'); showProgress(false); retryBtn?.classList.remove('hidden'); }
  }
  async function retry(){ if(!lastZipBlob||!lastPayloadBuilder||!lastZipFilename){ toast('재시도할 데이터가 없습니다.'); return; } retryBtn?.classList.add('hidden'); showProgress(true); setProgress(42,'재업로드'); setStatus('재업로드 중'); try{ const { data } = await submitBuiltZip({ zipBlob:lastZipBlob, zipFilename:lastZipFilename, payloadBuilder:lastPayloadBuilder, onProgress:(p,l)=>setProgress(p,l||'재업로드'), onStatus:setStatus }); setProgress(88,'다운로드'); await downloadWithFallback({ blob:lastZipBlob, filename:lastZipFilename, onStatus:setStatus, onToast:(m)=>toast(m) }); setProgress(100,'완료'); setTimeout(()=>showProgress(false),1200); toast('재제출 & 다운로드 완료','bg-green-600'); }catch(e){ setStatus('재시도 실패: '+e.message); toast('재시도 실패: '+e.message,'bg-red-600'); retryBtn?.classList.remove('hidden'); showProgress(false); } }
  downloadBtn?.addEventListener('click', attempt); retryBtn?.addEventListener('click', retry); printBtn?.addEventListener('click', printReport); previewBtn?.addEventListener('click', openPreview);
}

function bindClear(){ const clearBtn=qs('clear_inputs_btn'); if(!clearBtn) return; clearBtn.addEventListener('click',()=>{ if(!window.confirm('입력한 내용을 모두 삭제하시겠어요?')) return; try{ localStorage.removeItem(STORAGE_KEY); localStorage.removeItem(SUBMIT_META_KEY);}catch{} document.querySelectorAll('#tab4 input,#tab4 textarea').forEach(el=>el.value=''); const sc=qs('sources_container'); if(sc) sc.innerHTML=''; saveAll(); const sc2=qs('sources_container'); if(sc2) renderSourcesPreview(sc2); toast('입력 내용이 삭제되었습니다.','bg-red-600'); }); }

export function initSession2(){
  bindTabs();
  bindAccordions();
  loadSubmitMeta();
  loadAll();
...
document.addEventListener('DOMContentLoaded', initSession2);
