// modules/gate.js - 공개 시점 링크 표시 + 교사용 key 유지
  ...
  fetch('/api/time',{cache:'no-store'})
    ...
    .then(data=>{
      ...
      if(unlocked){
        ...
        show(s1); show(s2); hide(notice); s1.setAttribute('aria-disabled','false'); s2.setAttribute('aria-disabled','false');
      }else{
        hide(s1); hide(s2); show(notice); s1.setAttribute('aria-disabled','true'); s2.setAttribute('aria-disabled','true');
      }
    }).catch(()=>{ hide(s1); hide(s2); show(notice); });
}
